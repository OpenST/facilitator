...
7. `Simple Token balance on origin chain` is required to stake bounty in the gateway or to mint gas for auxiliary chain. `gas on auxiliary chain` is required to stake bounty for redeem 
   and unstake request.
8. Mosaic commands need a signer for signing transactions. Refer [here](https://github.com/mosaicdao/mosaic-chains#signing-of-transaction) for setting up signer for geth and parity node.     
